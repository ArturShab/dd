...
export async function POST(request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const existingSpecValues: Array<{ specID: number }> = await prisma.specValue.findMany({
      where: {
        productID: productId,
        specID: { in: specIDs },
      },
    ...
    return NextResponse.json({ added: createdSpecs.length, specValues: createdSpecs }, { status: 201 });
  } catch (error) {
